  ...
  <Story name='基础用法' height='420px'>
    {{
      ...
      methods:{
        changeBlocks(v){
          console.log(v)
          const info = v.map(({type,paths})=>{
            console.log(type)
            let o = {}
            if(type==='ellipse'){
              o = {type,data:JSON.stringify(this.path2ellipse(paths))}
            }else if(type==='rect'){
              o = {type,data:JSON.stringify(this.path2rect(paths))}
            }else {
        ...
        path2ellipse(paths){
          return {
            cx:(paths[0][0]+paths[3][0])/2, 
            cx:(paths[0][1]+paths[1][1])/2,
            rx:(paths[3][0]-paths[0][0])/2,
            ry:(paths[1][1]-paths[0][1])/2
          }
        },
        path2rect(paths){
          return {
            left:paths[0][0],
            top:paths[0][1],
            width:paths[3][0]-paths[0][0],
            height:paths[1][1]-paths[0][1],
          }
        },
        selectTool(t){
          this.state = t
